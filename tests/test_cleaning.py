...
def test_extrai_numero_da_categoria():
    ...
    resultado = extract_category_id(s, "location")
    assert resultado.tolist() == [118, 9, 1126]
...
def test_satelites_ficam_restritas_aos_incidentes_rotulados(clean):
    ...
    for nome, df in clean.as_dict().items():
        assert set(df["incident_id"]) <= ids, f"{nome} extrapolou o universo"
...
def test_nao_restou_nenhuma_coluna_de_texto(clean):
    ...
    for nome, df in clean.as_dict().items():
        assert not any(str(t) == "string" for t in df.dtypes), f"{nome} tem coluna textual"
...
def test_conteudo_bate_com_a_origem(clean):
    ...
    esperado = sorted(train["location"].str.removeprefix("location ").astype(int))
    assert sorted(clean.incidents["location_id"]) == esperado
...
def test_conjunto_minimo_e_valido():
    validate_clean_data(_clean_minimo())


def test_detecta_incident_id_duplicado():
    c = _clean_minimo()
    incidents = pd.concat([c.incidents, c.incidents.iloc[[0]]], ignore_index=True)
    with pytest.raises(ValueError, match="duplicado"):
        validate_clean_data(CleanData(**{**c.as_dict(), "incidents": incidents}))


def test_detecta_severidade_faltando_para_um_incidente():
    c = _clean_minimo()
    sev = c.incident_severity.iloc[[0]]
    with pytest.raises(ValueError, match="1 linha por incidente"):
        validate_clean_data(CleanData(**{**c.as_dict(), "incident_severity": sev}))
...
def test_detecta_ponte_apontando_para_incidente_inexistente():
    ...
    with pytest.raises(ValueError, match="inexistente"):
        validate_clean_data(CleanData(**{**c.as_dict(), "incident_events": ev}))


def test_detecta_par_duplicado_na_ponte():
    """Um par (incidente, evento) repetido inflaria qualquer contagem por evento."""
    c = _clean_minimo()
    ev = pd.DataFrame({"incident_id": [1, 1], "event_type_id": [11, 11]})
